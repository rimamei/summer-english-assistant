...
export default function MicAudioTest(): JSX.Element {
  ...
  const drawAudioWaveform = (): void => {
    ...
    if (outputWaveformRef.current && audioCanvasCtxRef.current && audioVisualizerRef.current) {
  ...
  const handlePlaySound = async (): Promise<void> => {
    try {
      ...
      const synth = new Tone.Synth().connect(outputWaveformRef.current as Tone.ToneAudioNode); // Assert as ToneAudioNode
  ...
  useEffect(() => {
    ...
    if (audioVisualizerRef.current) {
      ...
      audioVisualizerRef.current.height = audioVisualizerRef.current.offsetHeight;
    }
    ...
    return () => {
      ...
      if (audioDrawVisualRef.current) cancelAnimationFrame(audioDrawVisualRef.current);
      if (micDrawVisualRef.current) cancelAnimationFrame(micDrawVisualRef.current);
    };
